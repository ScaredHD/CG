...
Light light(vec3(1.0f, 1.0f, 1.0f), vec3(1.0f));
...
int main() {
    ...
    Shader gouraudShader("../shader/object_gouraud.vs", "../shader/object_gouraud.fs");
    Shader lightShader("../shader/light.vs", "../shader/light.fs");

    Model sphere1("../model/sphere.obj");
    Model sphere2("../model/sphere.obj");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        auto model = scale(mat4(1.0f), vec3(0.4f));
        ...
        model = scale(mat4(1.0f), vec3(0.4f));
        model = translate(model, vec3(2.0f, 0.0f, 0.0f));
        gouraudShader.setMat4("model", model);
        gouraudShader.setMat4("view", cam.viewMatrix());
        gouraudShader.setMat4("projection", cam.projectionMatrix());
        gouraudShader.setVec3("lightInWorld", light.pos);
        gouraudShader.setVec3("lightColor", light.color);
        gouraudShader.setVec3("objectColor", vec3(0.5f, 0.3f, 0.1f));

        sphere2.draw(gouraudShader);
...
GLFWwindow* init() {
    ...
    GLFWwindow* window =
        glfwCreateWindow(800, 600, "Gouraud & Phong shading", nullptr, nullptr);
    if (!window) {
