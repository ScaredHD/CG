...
Light light(vec3(0.0f, 5.0f, 0.0f), vec3(1.0f));
...
int main() {
    ...
    Model model("../model/teapot/teapot.obj");
    model.pos = vec3(0.0f);
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        glUniformMatrix4fv(glGetUniformLocation(outlineShader.id, "model"), 1,
                           GL_FALSE, value_ptr(modelMatrix));
        cam.sendToShader(outlineShader, "view", "projection");
        ...
        light.sendToShader(shader, "lightPos", "lightColor");
        model.draw(shader);
